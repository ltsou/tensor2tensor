...
class Transformer(t2t_model.T2TModel):
  ...
  def body(self, features):
    ...
    if inputs is not None:
      encoder_output, encoder_decoder_attention_bias = self._initialize_cache(features)
  ...
  def _maybe_tile_encoding(self, t, label, getting_samples=False):
    if t is not None:
      self.mrt_cache[label] = t
    else:
  ...
  def _initialize_cache(self, features, getting_samples=False):
    ...
    enc_dec_attn_bias = self._maybe_tile_encoding(enc_dec_attn_bias,
                                                  'encoder_decoder_attention_bias',
                                                  getting_samples)
    return enc_out, enc_dec_attn_bias
